...
client.connect('$REMOTE_IP', username='$REMOTE_USER', password='$REMOTE_PASSWORD', timeout=30)
...
cmd_db_push() {
    ...
    ssh_cmd "docker exec $DB_CONTAINER psql -U $DB_USER -c 'DROP DATABASE IF EXISTS $DB_NAME;'"
    ssh_cmd "docker exec $DB_CONTAINER psql -U $DB_USER -c 'CREATE DATABASE $DB_NAME OWNER $DB_USER;'"
...
cmd_media_pull() {
    ...
    docker cp "$media_tar" ${LOCAL_WEB_CONTAINER}:/tmp/media.tar.gz
    docker exec ${LOCAL_WEB_CONTAINER} tar -xzf /tmp/media.tar.gz -C /app/
...
cmd_media_push() {
    ...
    docker exec ${LOCAL_WEB_CONTAINER} tar -czf /tmp/media.tar.gz -C /app media
    docker cp ${LOCAL_WEB_CONTAINER}:/tmp/media.tar.gz "$media_tar"
